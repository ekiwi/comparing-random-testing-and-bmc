...
from scripts.benchmark import load_benchmark
...
def parse():
  ...
  return load_benchmark(args.benchmark), Path(args.input), Path(args.to), args.overwrite, args.skip_sim, args.source
...
def build_sim():
  cmd = ['verilator'] + _verilator_args + ['TileAndMemTop.sv']
...
def main():
  benchmark, inp, to, overwrite, skip_sim, source = parse()
