...
import firrtl.{AnnotationSeq, CircuitState, EmittedCircuitAnnotation}
...
object Compiler {
  ...
  def fromChisel(dutGen: => RawModule, annos: AnnotationSeq): CircuitState = {
    val (highFirrtl, _) = elaborate(() => dutGen, annos)
    val lowFirrtl = toLowFirrtl(highFirrtl, Seq())
    lowFirrtl
  }
  def fromChisel(dutGen: => RawModule, targetDir: TargetDirAnnotation): CircuitState =
    fromChisel(dutGen, Seq(targetDir))
}
